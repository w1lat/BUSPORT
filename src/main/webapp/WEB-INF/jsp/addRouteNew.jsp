...
<div class="generic-container" align="middle">
    ...
    <div class="row">
        ...
        <div class="row">
            <div class="form-group col-md-12">
                <label class="col-md-3 control-lable" for="arrival_station">Choose station of arriving new</label>

                <div class="col-md-7">
                    <select name="wayPoints" id="arrival_station">
                        <c:forEach items="${wayPointsList}" var="wayPoint">
                            <%--<option value="${station.id}"><c:out value="${station.stationCode} ${station.stationName}"/></option>--%>
                            <option value="${wayPoint.station}" ${wayPoint.station.id == route.wayPoints[fn:length(route.wayPoints)-1].id ? 'selected="selected"' : ''}>${wayPoint.station.stationCode} ${wayPoint.station.stationName}</option>
                        </c:forEach>
                    </select>
                </div>
            </div>
