    ...
    <form:form method="POST" modelAttribute="route" class="form-horizontal">
                ...
                <div class="col-md-7">
                    <select name="driver" id="driverList">
                        <c:forEach items="${driverList}" var="driver">
                            <option value="${driver.id}"><c:out
                                    value="${driver.name} ${driver.lastName} ${driver.surName}"/></option>
                        </c:forEach>
                    </select>
                </div>
            </div>
        ...
        <div class="row">
            <div class="form-group col-md-12">
                ...
                <div class="col-md-7">
                    <select name="driver" id="busList">
                        <c:forEach items="${busList}" var="bus">
                            <option value="${bus.id}"><c:out
                                    value="${bus.model} ${bus.registryNumber}"/></option>
                        </c:forEach>
                    </select>
                </div>
            </div>
                    ...
                    <select name="stations" id="arrival_station">
                        <c:forEach items="${stationList}" var="station">
                            <option value="${station.id}"><c:out
                                    value="${station.stationCode} ${station.stationName}"/></option>
                        </c:forEach>
                    </select>
                </div>
            </div>
